...
function isMap(fn) {
    return typeof fn === 'function' && !libUtil.isParser(fn);
}
...
function all() {
    ...
    var map = lastIsMap ? array[length - 1] : null;
    ...
    return libUtil.makeParser(function allParser(source) {
        ...
        if (map != null) mapR = map.apply(null, rs);
...
function any() {
    ...
    return libUtil.makeParser(function anyParser(source) {
        var rs = new Array(parsers.length);
        for (var i = 0; i < parsers.length; ++i) {
            var parser = parsers[i];
            var map = maps[i];
...
function oneOrMore(parser, map) {
    ...
    return libUtil.makeParser(function oneOrMoreParser(source) {
        ...
        var mapR = rs.map(function (v_) { return v_.result; });
        if (map != null) mapR = map(mapR);
